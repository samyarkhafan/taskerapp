<template>
  <div class="toast-container position-fixed bottom-0 end-0 p-3">
    <div id="toast" class="toast bg-success">
      ...
      <div class="toast-body">Task edited successfully.</div>
    </div>
  </div>
...
export default {
  ...
  beforeMount() {
    const obj = localStorage.getItem("task");
    var task = JSON.parse(obj);
    this.id = task.id;
    this.name = task.name;
    this.desc = task.desc;
    this.is_interval = task.is_interval;
    this.interval_days = task.interval_days;
    this.activates_on = task.activates_on;
  ...
  methods: {
    sendData() {
      ...
      if (!this.errors.length) {
        axios
          .put("http://127.0.0.1:8000/api/v1/tasks/" + this.id + "/", data)
          .then(() => {
            toast.show();
            setTimeout(() => {
